...
const store = createStore({
    ...
    getters: {
        ...
        checkRoadNameNumber: (state) => (roadNameNumber) => {
            ...
            let roadName = rn[0]
            let roadNumber = rn[1]
            return state.rs.find(rs => (rs.roadName === roadName && rs.roadNumber === roadNumber))
        }
    },
    mutations: {
        ...
        updateRsRfid(state, new_rs) {
            console.log(new_rs)
            let rsRfid = state.rsRfid.filter(rsRfid => rsRfid.rfid === new_rs.rfid)[0]
        ...
        updateAarCode(state, aar) {
            state.aarCodes = state.aarCodes.filter(aarCodes => aarCodes._id !== aar._id)
    ...
    actions: {
        ...
        async addNewRs({ commit }, new_rs) {
            ...
            RsService.getRsRoad(new_rs.roadName + "-" + new_rs.roadNumber).then(response => {
        ...
        updateRs({ commit }, rs) {
            ...
            if (!rs._id) {
                RsService.getRsRoad(rs.roadName + "-" + rs.roadNumber).then(response => {
