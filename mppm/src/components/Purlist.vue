  ...
  methods: {
    async getPurlist () {
      ...
      for (i = 0; i < response.data.purchases.length; i++) {
        this.purchases[i].date = this.formatDate(response.data.purchases[i].date)
        this.purchases[i].unitcost = formatter.format(response.data.purchases[i].unitcost)
      }
    },
    async deletePur (id) {
